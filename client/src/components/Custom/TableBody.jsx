	...
	return (
		<>
				...
				{loading ? (
					...
					currentRows.map((row, rowIndex) => (
						<motion.tr
							...
							{columns.map((column, columnIndex) => (
								<td
									key={`cell-${row.id || rowIndex}-${
										column.key || columnIndex
									}`}
									className={`p-2 md:p-3 text-left text-xs md:text-sm font-medium text-neutral-dark ${
										column.hiddenOnMobile ? "hidden md:table-cell" : ""
									} ${column.width || "w-auto"}`}>
					...
					<tr>
						<td
							...
							className="p-3 text-center text-neutral-dark text-xs md:text-sm">
							No data available
						</td>
					</tr>
				)}
			...
			<div className="md:hidden space-y-3">
				{loading ? (
					Array.from({ length: 5 }).map((_, index) => (
						<MobileSkeletonLoader key={index} />
					))
				) : currentRows.length > 0 ? (
					currentRows.map((row, rowIndex) => (
						<motion.div
							...
							<div className="flex justify-between items-center mb-3 border-b pb-2 border-gray-200">
								{columns.map((column, columnIndex) =>
									column.key === "select" || column.key === "actions" ? (
										<div
											key={`cell-mobile-top-${row.id || rowIndex}-${
												column.key || columnIndex
											}`}
							...
							<div className="space-y-2">
								{columns.map((column, columnIndex) =>
									column.key !== "select" && column.key !== "actions" ? (
										<div
											key={`cell-mobile-bottom-${row.id || rowIndex}-${
												column.key || columnIndex
											}`}
